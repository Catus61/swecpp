...
def cal(time):
    ...
    P_error = math.sqrt(sum(((P_syn-P_msr)/(P_max-P_min))**2) * 1.0 / NN)
    u_error = math.sqrt(sum(((u_syn-u_msr)/(u_max-u_min))**2) * 1.0 / NN)
    v_error = math.sqrt(sum(((v_syn-v_msr)/(v_max-v_min))**2) * 1.0 / NN)
