...
void RK4drift(struct drifter *ptdrifter, size_t ndr, const double *fields, const double *parameters, const double *forcing, int xdim, int ydim, double dx, double dy, int **neighbors, const unsigned int *lat, const unsigned *lon, double dt){
    ...
    temp_ptdrifter = new struct drifter[ndr];
    temp_fields_rk = new double[3 * tdim];
    temp_fields_dots_rk = new double[3 * tdim];
   ...
   for(i = 0; i < 3 * tdim; i++)
    ...
    delete[]temp_fields_dots_rk;
    delete[]temp_fields_rk;
    delete[]temp_ptdrifter;
...
//    double duration;
    ...
    u = new double[(xdim + 1) * (ydim + 1)];
    v = new double[(xdim + 1) * (ydim + 1)];
    P = new double[(xdim + 1) * (ydim + 1)];
