...
void Fcalc(double *fields_dot, const double *fields, const double *parameters, const double *forcing, int xdim, int ydim, double dx, double dy, const int **n, const unsigned int *lat, const unsigned int *lon) {
    ...
    for (i = 0; i < tdim; i++) {
        u_dot[i] = S8 * (eta[n[i][1]] + eta[i]) * (V[n[i][1]] + V[n[n[i][2]][1]] + V[n[i][2]] + V[i]) -
                   SX * (H[i] - H[n[i][2]]);

        v_dot[i] = -S8 * (eta[n[i][0]] + eta[i]) * (U[n[i][0]] + U[i] + U[n[i][3]] + U[n[n[i][3]][0]]) -
                   SY * (H[i] - H[n[i][3]]);
    ...
    for (i = 0; i < tdim; i++) {
        ...
        u_dot[i] += dissipationTerm * ((SX * SX) * (u[n[i][0]] + u[n[i][2]] - 2.0 * u[i]) +
                                       (SY * SY) * (u[n[i][1]] + u[n[i][3]] - 2.0 * u[i]));

        v_dot[i] += dissipationTerm * ((SX * SX) * (v[n[i][0]] + v[n[i][2]] - 2.0 * v[i]) +
...
void Fcalc_ad(adouble *fields_dot, const adouble *fields, const double *parameters, const double *forcing, int xdim, int ydim, double dx, double dy, const int **n, const unsigned int *lat, const unsigned int *lon)
{
    int i;
    ...
    const adouble *u = fields;
    const adouble *v = fields + tdim;
    const adouble *P = fields + 2 * tdim;

    adouble *u_dot = fields_dot;
    adouble *v_dot = fields_dot + tdim;
    adouble *P_dot = fields_dot + 2 * tdim;
    ...
    U = calloc(tdim, sizeof(adouble));
    V = calloc(tdim, sizeof(adouble));
    H = calloc(tdim, sizeof(adouble));
    eta = calloc(tdim, sizeof(adouble));
...
void RK4(double *fields_dot, double *fields, const double *parameters, const double *forcing, int xdim, int ydim, double dx, double dy, double dt, const int **neighbors, const unsigned int *lat, const unsigned int *lon){
    ...
    */
    ...
    Fcalc(fields_dot, fields, parameters, forcing, xdim, ydim, dx, dy, neighbors, lat, lon);
    ...
    Fcalc(temp_dots_rk, temp_fields_rk, parameters, forcing, xdim, ydim, dx, dy, neighbors, lat, lon);
...
void RK4_ad(adouble *fields_dot, adouble *fields, const double *parameters, const double *forcing, int xdim, int ydim, double dx, double dy, double dt, const int **neighbors, const unsigned int *lat, const unsigned int *lon){
    ...
    */
    ...
    //temp variables
    adouble *temp_dot = new[3 * tdim];
    adouble *temp_fields = new[3 * tdim];


    adouble fields_in = fields;
    adouble fields_dot_out = fields_dot;

    Fcalc(fields_dots_out, fields_in, parameters, forcing, xdim, ydim, dx, dy, neighbors, lat, lon);

    for (i=0; i < 3 * tdim; i++)
    {
        temp_fields_rk[i] = fields_in[i] + 0.5 * dt * fields_dot_out[i];
    }

    Fcalc(temp_dots, temp_fields, parameters, forcing, xdim, ydim, dx, dy, neighbors, lat, lon);

    for (i=0; i < 3 * tdim; i++)
    {
        fields_dot_out[i] += 2 * temp_dots[i];
    }
    ...
    for (i=0; i < 3 * tdim; i++)
    {
        temp_fields[i] = fields_in[i] + 0.5 * dt * temp_dots_rk[i];
    }
    ...
    for (i=0; i < 3 * tdim; i++)
    {
        temp_fields[i] = fields_in[i] + dt * temp_dots_rk[i];
    }
